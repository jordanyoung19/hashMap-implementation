...
private:
    ...
    // adding more private members
    Node** bucketList;
    Node** newBucketList;
    unsigned int sz;
    unsigned int cap;

    // helper functions that users will not need to see
    void createFirstArray(Node** &bucketList, unsigned int size);
    void addKeyValueToBucket(const std::string& key, const std::string& value, int index);
    void addKeyValueToNewBucket(const std::string& key, const std::string& value, int index);
    void bucketListCopy(Node** target, Node** source, unsigned int cap);
    void rehashElements(unsigned int cap, unsigned int newCap);
};
